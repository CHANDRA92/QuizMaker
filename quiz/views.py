...
def admin_add_course_view(request):
    ...
    #teacher1= User.objects.get(id=request.user.id).pk
    try:
        teacher1 = models.Teacher.objects.get(user=request.user.id).pk
    except teacher1.DoesNotExist:
        return HttpResponseRedirect('/teacher/teachersignup')
    print(teacher1)
    val=q.generate_unique_course_code(request)
    #print(val)
    courseForm = forms.CourseForm(initial={'course_code': val,'teacher_id':teacher1})
    
    if request.method=='POST':
        courseForm=forms.CourseForm(request.POST,initial={'course_code': val,'teacher_id':teacher1})
...
@login_required(login_url='adminlogin')
def view_quiz_view(request,pk):
    courses = models.testdetails.objects.all().filter(course_id=pk)
    return render(request,'quiz/view_quiz.html',{'courses':courses})


@login_required(login_url='adminlogin')
def view_question_view(request,pk):
    questions=models.Question.objects.all().filter(testno=pk)
...
def admin_view_marks_view(request,pk):
    courses = models.Result.objects.all().filter(student=pk)
